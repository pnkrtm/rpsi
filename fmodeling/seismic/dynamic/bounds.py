...
def calculate_bounds(model, rays, owt):
    """
    ...
    boundaries = np.concatenate([r.boundaries.values() for r in rays_])
    indexes = np.concatenate([r.boundaries.keys() for r in rays_])
    bound_indexes = [b["boundary_index"] for b in boundaries]
    bound_types = [b["boundary_type"] for b in boundaries]
    angles = [rays_[ri].get_boundary_angle(i) for ri, i in zip(ray_indexes, indexes)]

    assert len(ray_indexes) == len(boundaries) == len(indexes) == len(bound_indexes) == len(bound_types) == len(angles)

    boundary_coeffs = [np.NAN]*len(boundaries)

    unique_bounds = list(set(bound_types))

    vp1_arr = [model.get_single_param(param_name='vp', index_start=bi - 1, index_finish=bi)[0] for bi in
               bound_indexes]
    vs1_arr = [model.get_single_param(param_name='vs', index_start=bi - 1, index_finish=bi)[0] for bi in
               bound_indexes]
    rho1_arr = [model.get_single_param(param_name='rho', index_start=bi - 1, index_finish=bi)[0] for bi in
                bound_indexes]

    vp2_arr = [model.get_single_param(param_name='vp', index_start=bi, index_finish=bi + 1)[0] for bi in
               bound_indexes]
    vs2_arr = [model.get_single_param(param_name='vs', index_start=bi, index_finish=bi + 1)[0] for bi in
               bound_indexes]
    rho2_arr = [model.get_single_param(param_name='rho', index_start=bi, index_finish=bi + 1)[0] for bi in
                bound_indexes]
