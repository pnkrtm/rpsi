...
from obspy_edited.header import (BINARY_FILE_HEADER_FORMAT,
...
from obspy_edited.unpack import OnTheFlyDataUnpacker
from obspy_edited.util import unpack_header_value
...
class SEGYFile(object):
    ...
    def __init__(self, file=None, endian=None, textual_header_encoding=None,
                 unpack_headers=False, headonly=False, read_traces=True, hdf_writer = None):
        """
        Class that internally handles SEG Y files.
        ...
        :param read_traces: Data traces will only be read if this is set to
            ...
            ``False``.
        """
        # if not hdf_writer is None:
        #     self.hdf_writer = hdf_writer
        ...
        while True:
            ...
            try:
                ...
                if yield_each_trace:
                    ...
                    if self.hdf_writer == 0:
                        self.traces.append(trace)
                    else:
                        if headonly and not self.hdf_writer.need_writing:
                            self.hdf_writer.count_of_traces += 1
                        elif self.hdf_writer.need_writing:
                            self.hdf_writer.add_trace(trace, i)
                        else:
                            self.traces.append(trace)
...
def _read_segy(file, endian=None, textual_header_encoding=None,
               unpack_headers=False, headonly=False, hdf_writer=None):
    """
    ...
    if not hasattr(file, 'read') or not hasattr(file, 'tell') or not \
        ...
        with open(file, 'rb') as open_file:
            return _internal_read_segy(
                ...
                unpack_headers=unpack_headers, headonly=headonly, hdf_writer=hdf_writer)
    # Otherwise just read it.
    return _internal_read_segy(file, endian=endian,
                               textual_header_encoding=textual_header_encoding,
                               unpack_headers=unpack_headers,
                               headonly=headonly, hdf_writer=hdf_writer)
